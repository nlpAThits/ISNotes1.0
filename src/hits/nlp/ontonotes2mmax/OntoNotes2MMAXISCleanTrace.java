...
public class OntoNotes2MMAXISCleanTrace {
    ...
    private LinkedHashMap<Integer, String> _newTokens;
    private LinkedHashMap<Integer, Integer> _tokenMap;
    protected String currentDocNameSpace;

    public OntoNotes2MMAXISCleanTrace(File sourceDir, File targetDir, File resourceDir, File entityAnnotationDir) {
        ...
        _newTokens = new LinkedHashMap<Integer, String>();
        _tokenMap = new LinkedHashMap<Integer, Integer>();
        loadFiles();
        run();
    }
    ...
    private void cleanTrace() {
        int oldID = 0; //token counted from 1
        int newID = 0;
        for (Entry<Integer, String> oldToken : _tokens.entrySet()) {
            String tokenStr = oldToken.getValue();
            if (!(tokenStr.matches("((\\*[A-Z]+?\\*(-[0-9]+)?)|(\\*?\\*(-[0-9]+)?)|(\\*(-[0-9]+)?))|(0)|(\\*\\?\\*)"))) {
                _newTokens.put(newID, tokenStr);
                _tokenMap.put(oldID, newID);
                newID++;
            }
            oldID++;
        }
    ...
    private LinkedList<Integer> getNewSpans(Integer startID, Integer endID) {
        LinkedList<Integer> mapResult = new LinkedList<Integer>();
        if (_tokenMap.containsKey(startID)) {
            mapResult.add(_tokenMap.get(startID));
        }
        if (_tokenMap.containsKey(endID)) {
            mapResult.add(_tokenMap.get(endID));
        }
        return mapResult;
    }
    ...
    private void writeSentenceMarkables() throws IOException {
        ...
        for (int sentId : _sents.keySet()) {
            ...
            LinkedList<Integer> newSpan = getNewSpans(sentStart, sentEnd);
            String span = "";
            if (newSpan.size() == 1 || newSpan.get(0)==newSpan.get(1)) {
                span = "word_" + (newSpan.get(0) + 1);
            } else {
                span = "word_" + (newSpan.get(0) + 1) + "..word_" + (newSpan.get(1) + 1);
            }
    ...
    private void writeCorefMarkables() throws IOException {
        ...
        for (int setId : _corefs.keySet()) {
            for (int[] menRange : _corefs.get(setId)) {
                ...
                int End = menRange[1];
                String span = "";
                LinkedList<Integer> newSpan = getNewSpans(Start, End);
                if (newSpan.size() == 1 || newSpan.get(0)==newSpan.get(1)) {
                    span = "word_" + (newSpan.get(0) + 1);
                } else {
                    span = "word_" + (newSpan.get(0) + 1) + "..word_" + (newSpan.get(1) + 1);
    ...
    public void writeBaseData() throws IOException {
        ...
        for (int wId : _newTokens.keySet()) {
            wordWriter.write("<word id=\"word_" + (wId + 1) + "\">" + _newTokens.get(wId) + "</word>" + "\n");
        }
    ...
    public static void main(String[] args) {
        try {
            if (args.length == 4) {
                ...
                new OntoNotes2MMAXISCleanTrace(file_ontonotesONFDir, file_targetDir, file_resourceDir, file_ISAnnotationDir);
            } else {
                System.err.println("please specify correct path!");
            }

//            new OntoNotes2MMAXISCleanTrace(
//                    new File("/data/nlp/houyg/corpora/ISRelease/OntoNotesOriginal"),
//                    new File("/data/nlp/houyg/corpora/ISRelease/withCorefClean"),
//                    new File("/data/nlp/houyg/corpora/ISRelease/resourceFile"),
//                    new File("/data/nlp/houyg/corpora/ISRelease/ISAnnotationWithoutTrace"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
